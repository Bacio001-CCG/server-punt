"use client";
import { MdLocationOn } from "react-icons/md";
import { useTranslations } from "next-intl";
import CountrySelect from "./country-select";

export default function Delivery({
    setSendingMethod,
    sendingMethod,
    countryCode,
    onCountryChange,
}: {
    setSendingMethod: (method: "delivery" | "pickup") => void;
    sendingMethod: "delivery" | "pickup";
    countryCode: string;
    onCountryChange: (code: string) => void;
}) {
    const t = useTranslations("checkout");

    return (
        <div className="flex flex-col gap-5">
            <h2 className="text-3xl font-bold mb-2">{t("delivery")}</h2>
            <div className="grid grid-cols-2 w-full border-border border rounded-lg divide-x divide-x-border h-10">
                <button
                    type="button"
                    onClick={() => setSendingMethod("delivery")}
                    className={`w-full ${
                        sendingMethod === "delivery" && "bg-accent"
                    }`}
                >
                    {t("deliver")}
                </button>
                <button
                    type="button"
                    onClick={() => setSendingMethod("pickup")}
                    className={`w-full ${
                        sendingMethod === "pickup" && "bg-accent"
                    }`}
                >
                    {t("pickup")}
                </button>
            </div>
            <input type="hidden" name="delivery_method" value={sendingMethod} />
            {sendingMethod === "delivery" ? (
                <DeliveryDelivery
                    countryCode={countryCode}
                    onCountryChange={onCountryChange}
                />
            ) : (
                <DeliveryPickup />
            )}
        </div>
    );
}

function DeliveryPickup() {
    const t = useTranslations("checkout");

    return (
        <a
            href="https://www.google.com/maps/dir//ServerPunt,+Kraaivenstraat+36,+07+Tilburg/@51.579652,5.0627759,651m/data=!3m1!1e3!4m17!1m7!3m6!1s0x47c6950071838207:0xe86114d2bfc5f69a!2sServerPunt!8m2!3d51.579652!4d5.0653508!16s%2Fg%2F11mlgqbr3k!4m8!1m0!1m5!1m1!1s0x47c6950071838207:0xe86114d2bfc5f69a!2m2!1d5.0653508!2d51.579652!3e0?entry=ttu&g_ep=EgoyMDI1MTEwNC4xIKXMDSoASAFQAw%3D%3D"
            target="_blank"
            rel="noopener noreferrer"
            className="p-3 rounded-lg border border-border bg-white flex justify-between hover:bg-accent duration-300 transition-all"
        >
            <div>
                <h3 className="text-lg font-semibold">{t("pickupAddress")}</h3>
                <p className="text-gray-500">{t("pickupInfo")}</p>
            </div>
            <MdLocationOn size={40} className="my-auto" />
        </a>
    );
}

function DeliveryDelivery({
    countryCode,
    onCountryChange,
}: {
    countryCode: string;
    onCountryChange: (code: string) => void;
}) {
    const t = useTranslations("checkout");

    return (
        <>
            <div>
                <label htmlFor="delivery.country">{t("country")}</label>
                <CountrySelect
                    id="delivery.country"
                    name="delivery.country"
                    value={countryCode}
                    onChange={onCountryChange}
                />
            </div>
            <div className="grid grid-cols-2 gap-5 w-full">
                <div>
                    <label htmlFor="delivery.firstname">{t("firstname")}</label>
                    <input
                        type="text"
                        id="delivery.firstname"
                        className="block w-full border border-gray-300 rounded-md p-2 mt-1"
                        placeholder="John"
                        name="delivery.firstname"
                    />
                </div>
                <div>
                    <label htmlFor="delivery.lastname">{t("lastname")}</label>
                    <input
                        type="text"
                        id="delivery.lastname"
                        className="block w-full border border-gray-300 rounded-md p-2 mt-1"
                        placeholder="Doe"
                        name="delivery.lastname"
                    />
                </div>
            </div>
            <div>
                <label htmlFor="delivery.company">{t("companyOptional")}</label>
                <input
                    type="text"
                    id="delivery.company"
                    className="block w-full border border-gray-300 rounded-md p-2 mt-1"
                    placeholder="ServerPunt"
                    name="delivery.company"
                />
            </div>
            <div>
                <label htmlFor="delivery.address">{t("address")}</label>
                <input
                    type="text"
                    id="delivery.address"
                    className="block w-full border border-gray-300 rounded-md p-2 mt-1"
                    placeholder="Kraaivenstraat 36"
                    name="delivery.address"
                />
            </div>
            <div className="grid grid-cols-2 gap-5 w-full">
                <div>
                    <label htmlFor="delivery.postalcode">{t("postalcode")}</label>
                    <input
                        type="text"
                        id="delivery.postalcode"
                        className="block w-full border border-gray-300 rounded-md p-2 mt-1"
                        placeholder="5048AB"
                        name="delivery.postalcode"
                    />
                </div>
                <div>
                    <label htmlFor="delivery.city">{t("city")}</label>
                    <input
                        type="text"
                        id="delivery.city"
                        className="block w-full border border-gray-300 rounded-md p-2 mt-1"
                        placeholder="Tilburg"
                        name="delivery.city"
                    />
                </div>
            </div>
            <div>
                <label htmlFor="delivery.phonenumber">{t("phone")}</label>
                <input
                    type="tel"
                    id="delivery.phonenumber"
                    className="block w-full border border-gray-300 rounded-md p-2 mt-1"
                    placeholder="0612345678"
                    name="delivery.phonenumber"
                />
            </div>
            <div className="flex gap-2">
                <input type="checkbox" name="delivery.save" id="delivery.save" />
                <label htmlFor="delivery.save">{t("saveAddress")}</label>
            </div>
        </>
    );
}
